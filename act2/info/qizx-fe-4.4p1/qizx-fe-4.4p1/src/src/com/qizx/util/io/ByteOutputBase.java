    ...
    implements ByteOutput, Serializable
{
    ...
    {
        if(code < 0)
            throw new IllegalArgumentException("negative Vint "+code);
        if(code < 128) {
            ...
            buffer[bufSize + 1] = (byte) (code >> 16); 
            buffer[bufSize + 2] = (byte) (code >> 8); 
            buffer[bufSize + 3] = (byte) code;
            return bufSize + 4;
        }
        else { // 3 bits remaining
    ...
    {
        if(code < 0)
            throw new IllegalArgumentException("negative Vlong "+code);
        if(code < (1L << 28)) {
