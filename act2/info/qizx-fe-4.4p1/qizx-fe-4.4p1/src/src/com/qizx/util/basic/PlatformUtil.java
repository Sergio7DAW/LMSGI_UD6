...
public final class PlatformUtil
{
    ...
    public static void getEnvironment(HashMap env) {
        ...
        try {
            Process process;
            if (IS_WINDOWS) 
                process = Runtime.getRuntime().exec(new String[] { "cmd.exe", "/c", "set" });
            else 
                process = Runtime.getRuntime().exec(new String[] { "/bin/sh", "-c", "env" });
    ...
    public static int getIntSysProp(String name)
    {
        ...
        try {
            ...
            System.err.println("improper value of system property " + name +
                               ": should be integer");
            return -1;
        }
    }
