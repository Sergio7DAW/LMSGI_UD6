    ...
    implements FullTextPullStream
{
    ...
    private ArrayList<Chunk> snippetChunks;
...
//        }
        ...
        snippetChunks = new ArrayList<Chunk>();
    ...
    private Chunk getChunk(int index, ArrayList<Chunk> chunks)
    {
        return chunks.get(index);
    }
    
    private int countWords(ArrayList<Chunk> chunks)
    {
        int count = 0;
    ...
    private void storeChunk(int type, String text, int wordCount, int termPos)
    {
        ...
        if(type != FT_TERM)
            return;
        ArrayIntSet termSet = new ArrayIntSet(termPos);
