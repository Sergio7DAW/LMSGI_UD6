 ...
 * or a {@link Document}: see {@link Document#export(XMLPushStream)}
 */
    ...
    implements XMLPushStream /* redundant on purpose */
{
        ...
        throws DataModelException
    {
        ...
        if(autoDTD && !omitXmlDecl && node instanceof BasicNode) {
            ...
            if(dtdInfo != null) {
                dtdName = realString(dtdInfo[0]);
                dtdSystemId = realString(dtdInfo[1]);
                dtdPublicId = realString(dtdInfo[2]);
            }
        }
        super.putNodeCopy(node, copyNsMode);
    }

    private String realString(String s)
    {
        ...
        throws DataModelException
    {
        ...
        setOutput(out);
        boolean isDoc = node.getNodeNature() == Node.DOCUMENT;
        if (isDoc)
            putDocumentStart();
        putNodeCopy(node, 0);
        if (isDoc)
    ...
    protected void flushElementStart(boolean empty) throws DataModelException
    {
        ...
        if (firstElement) { 
            ...
            if (!omitXmlDecl &&
                 (dtdName != null || dtdPublicId != null || dtdSystemId != null))
            {
                print("<!DOCTYPE ");
                if(dtdName == null)
