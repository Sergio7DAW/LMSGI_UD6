...
{
    // MUST be at head!
    private static HashMap<String, String> internedStrings = new HashMap<String, String>();
     ...
     */
    public TreeMap<String, String> getMappings(boolean erase)
    {
    ...
    {
        TreeMap<String, String> prefs = getMappings(false);
        String[] result = new String[prefs.size()];
        int i = 0;
        for (Iterator<String> iter = prefs.keySet().iterator(); iter.hasNext();) {
            result[i++] = iter.next();
        }
        return result;
    }
    ...
    {
        TreeMap<String, String> map = getMappings(true);
...
//        ++size;
    ...
    {
        String r = internedStrings.get(s);
        if(r != null)
            return r;
